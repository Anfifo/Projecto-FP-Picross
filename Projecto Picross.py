...
#_____________________Construtor_______________________
...
# - - cria uma listas com listas a zero no interior - - #
...
# - - verifica se os tuplos contem tuplos - - #
...
#________________________ Selectores ___________________________________
...
def tabuleiro_celula(tabuleiro,coordenada):
    ...
    celula = tabuleiro[0][linha][coluna]     # vai buscar o valor na celula do tabuleiro
...
def tabuleiro_preenche_celula(tabuleiro,coordenada,inteiro):
    ...
    tabuleiro[0][linha-1][coluna-1] = inteiro    # altera o valor na posicao para o inteiro
...
#- - print das especificacoes das linhas no final - - 
        for i in range(max(dim_Elin)):
        
    # caso possua elemento no index faz print dele
            if len(E_lin[l])>=i+1:
                
                if i==max(dim_Elin)-1:
                    print('',E_lin[l][i],end='')
    
    # elemento de maior length define o tamanho
                else:
                    print('',E_lin[l][i],end='')    
            

    # caso contrario faz print de espacos 
            else:
                print(' ',end=' ')
...
def tabuleiro_completo(tabuleiro):
    ...
    if not (tabuleiro_preenchido(tabuleiro)):         #se nao estiver todo preenchido entao nao ta completo
        return False 

    # arranjar a coluna e lista em estudo
    ...
    # verificar se a coluna e lista em estudo seguem as especificacoes
        if not(\
...
# - - recebe os inputs da jogada e escreve com os limites ditos - - #
    if not e_tabuleiro(tabuleiro):
        raise ValueError('e_tabuleiro: argumentos invalidos')
    string = input ('Introduza a jogada\n- coordenada entre (1 : 1) e '+\
                coordenada_para_cadeia(cria_coordenada(tabuleiro_dimensoes(tabuleiro)[0],tabuleiro_dimensoes(tabuleiro)[0]))+' >> ')
    valor = input ("- valor >> ")


# - - avalia e trata do input para chegar a coordenada - - #
...
# - - calcula e retorna o valor da jogada - - # 
    coord0 = int(coords[0])
    coord1 = int(coords[1])
    nr_linhas = tabuleiro_dimensoes(tabuleiro)[0]
...
#__________________________ Funcao Principal _________________________
...
# - - game over se tiver preenchido erradamente - - #
...
# - - aplicar mudanca caso jogada valida - - #                
        if e_jogada(jogada):
