...
# avalia o input 
    teste = eval(string.replace(":",","))
    try:
        coordenada = cria_coordenada(int(teste[0]),int(teste[1]))
    except(ValueError):
        coordenada = cria_coordenada(teste[0],teste[1])
# verifica a avaliacao do input 
    if not (e_coordenada(coordenada)\
            and 0<coordenada[0]<= numero_colunas(tabuleiro)\
            and 0<coordenada[1]<= numero_linhas(tabuleiro)):
        return False 
